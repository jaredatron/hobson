...
Hobson.loadContent = function(callback){
  ...
  if (Hobson.isPageVisible()){
    ...
    request.success(function(html){
      $('#content').html(html).trigger('init');
...
// like document rady but runs everytime we refresh #content
$(document).delegate('#content', 'init', function(e){
  $("abbr.timeago").timeago();
});

$(function(){
  $('#content').trigger('init');
  if ($('body').is('.auto_refresh')) Hobson.autoRefresh();
});
